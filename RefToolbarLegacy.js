...
var option5="cite encyclopedia";
var option6="cite press release";
...
function refbuttons() {
  if (mwCustomEditButtons && document.getElementById('toolbar') ) {
    ...
    if (navigator.userAgent.indexOf('MSIE') == -1) {
      ...
      citemain.appendChild( addOption("citeWeb()", option1) );
      citemain.appendChild( addOption("citeNews()", option2) );
      citemain.appendChild( addOption("citeBook()", option3) );
      citemain.appendChild( addOption("citeJournal()", option4) );
      ...
      citeselect.innerHTML = '<input type="button" value=option1 onclick="citeWeb()" />'+
      '<input type="button" value=option2 onclick="citeNews()" />'+
      '<input type="button" value=option3 onclick="citeBook()" />'+
      '<input type="button" value=option4 onclick="citeJournal()" />'+
...
function citeNewsWeb(templatename) {
  ...
  var form = '<div id="citediv'+numforms+'">'+
    ...
    '<tr><td width="120"><label for="work">&nbsp;' + ((template == option2) ? 'Newspaper' : 'Work') + ': </label></td>'+
...
function citeBook() {
  ...
  form += '<tr><td><label for="coauthors">Coauthors: </label></td> \
	...
	\
	<input type="radio" tabindex=1 name="template" id="cite_book" value="cite_book" checked="1"><label for="cite_book">{{option3}}</label> <sup><a href="//en.wikipedia.org/wiki/Template:Cite_book" target="_blank">[doc]</a></sup> \
...
function makeBookCitationCode(callback) {
  ...
  if (document.getElementById('plain').checked) {
    ...
    if (match) {
      ...
      var citemid = match[2];
      var citeend = match[3];
      citemid = citemid.replace(/option3/, "Vancite book");
